...
def test_provision_mental_models_creates_one_entry_per_key(backend):
    builtins = (
        BuiltinModel(key="user-context", name="User Context", prompt="p1", version=1),
        BuiltinModel(key="project-context", name="Project Context", prompt="p2", version=1),
    )
...
def test_provision_mental_models_overwrites_on_repeat_call(backend):
    backend.provision_mental_models(
        BANK, (BuiltinModel(key="user-context", name="User Context", prompt="old", version=1),)
    )
    backend.provision_mental_models(
        BANK, (BuiltinModel(key="user-context", name="User Context", prompt="new", version=2),)
    )
...
def test_reset_clears_memories_and_mental_models(backend):
    ...
    backend.provision_mental_models(
        BANK, (BuiltinModel(key="user-context", name="User Context", prompt="p", version=1),)
    )
