...
| Tests | 634 unit/API passed, 2 deselected (integration; need live Hindsight), 2 e2e smokes (REST `scripts/smoke.sh`, MCP `scripts/mcp-smoke.py`) plus `scripts/e2e.py`'s scenario suite |
...
| Plan 7 (whole-repo review remediation: 3 critical + ~35 important findings from the 2026-08-23 five-slice review) | **complete for everything verifiable here** — `hide_parameters` (bound SQL params carried `bank_id` into the logs); control-character screening across 12 identifier sites in 6 files; the bank-id substring redaction pinned on all 28 call sites (the mutation survived a 504-test suite); one shared leak scanner, unanchored so it finally sees a `chunk_id`-embedded bank id; compose bound to loopback and the published master key removed from the README; CI that actually runs on `main`; `mcp-types` declared; the chart's CI job made real (it passed a chart with its master-key guard deleted); a public release workflow; `extra="forbid"`, page-size ceilings and content caps on both surfaces; MCP authenticating before validating and no longer advertising `recall`/`reflect` as read-only; `rename()` savepointed; the master key's rate limit made per-subject; split LLM timeouts. **Not verifiable here:** `release.yml` and CI's image job have never executed — no remote, no tag, no runner. **Dropped:** the three script-fix tasks (real bugs, but `smoke.sh`/`e2e.py`/`mcp-smoke.py` cannot be run without a live Hindsight, so they would have shipped unverified). See `docs/superpowers/plans/2026-08-23-07-whole-repo-review-remediation.md` |
