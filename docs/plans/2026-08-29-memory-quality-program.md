...
**Status:** Phase 5.5, its Phase 5.6 semantic repair and the Phase 5.7 measured splitter comparison are complete; see `docs/results/2026-09-02-memory-quality-phase-5-5.md`. Phase 5.5 proves local ACH sanitization must remain: official-only preprocessing leaked the planted canary. Delivery/compiler, reliability/ordering and host adapters remain `insufficient_evidence` for replacement.

Phase 5.6 run `dd364495-b7fa-4908-ab43-102c35e5cd27` repaired the prompt drift and completed 144 semantic observations plus five-judge adjudication. It still does not authorize a semantic replacement. ACH lost S01/S12/S15 to one provenance-frame contract failure and misrouted S03/S05/S06/S09/S14; S09 itself is not a valid Working State stimulus. Native extraction did not exercise pre-retain physical routing. The toy hybrid neither persisted bank projections nor measured isolation, hardcoded its scope/isolation gates and lost critical semantic units. Its apparent router pass and native's apparent non-inferiority are therefore `insufficient_evidence`, not adoption rulings.

Phase 5.7 run `fa142ba9-d706-4df8-b8ac-83764b8d480e` closes that minimum comparison. It repaired invalid-provenance blast radius candidate-by-candidate, replaced S09 in versioned corpus `semantic-v2`, measured non-zero elapsed time, and exercised a strict five-destination splitter with separate disposable User/Project banks and post-retain `original_text` inspection. The 144-observation run created and deleted exactly 192 banks; three independent judges covered all tuples.

The splitter fails adoption. Against repaired ACH it has repeatable additional losses in S07, S08, S09, S10 and S15, plus hard routing/contract failures. Native retain stores the sanitized source document in a shared bank and therefore fails physical scope isolation by measured bank contents, not by a variant-name constant. Accepted rulings are `semantic_extractor=keep` and `scope_router=keep`. ACH still has bounded baseline guards in S03, S06 and S14; address those as named quality debt without adding free-text heuristics or expanding the taxonomy. No production activation or Phase 0 mutation is authorized.
