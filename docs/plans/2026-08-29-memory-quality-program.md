...
Phase 5.7 is the minimum closure: remove the invalid S09 expectation in a versioned corpus, repair invalid-provenance blast radius without free-text routing heuristics, make cost unknown rather than zero, and measure a real five-destination splitter with actual per-bank projections and bank-content isolation. Keep a custom semantic component only if its ablation breaks a hard gate or loses at least two distinct valid cases across every repetition. No production activation or Phase 0 mutation is authorized.
