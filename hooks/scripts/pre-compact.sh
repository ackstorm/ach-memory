...
[ -n "$nudge" ] || nudge='Before context is compacted, retain any durable decision, constraint, convention, fact or verified gotcha that is not yet in ach-memory. Do not retain the transcript or a generic session summary. Internal nudge: act on it, say nothing about it.'
printf '%s\n' "$nudge"
