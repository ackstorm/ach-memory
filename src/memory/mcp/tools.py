...
def _run_working_state(ctx: Context, body_factory, call) -> ToolResult:
    ...
    one as an accidental side effect of a call that has nothing to do with
    Hindsight. Authentication, rate limiting, error mapping and activity
    tracking match `_run` exactly; only the bank/compact machinery is gone.

    Both callers of this helper are writes -- Working State has no read
    tool -- so the rate-limit gate below is unconditional, matching `_run`'s
    `is_write=True` writes (SPEC §20).
    """
    activity.new_call()
    try:
        with tool_session(ctx) as tc:
            ratelimit.check(tc.principal)
            # Same reasoning as _run: built inside the session, after
            # authentication, so an uncredentialed caller never reaches
            # pydantic's bounds as a free oracle for the request shape.
            body = body_factory()
