...
def create_project(
    ...
    owner = body.owner
    if owner is None:
        # Every caller has an identity now, operators included, so an unnamed
        # owner is simply the caller -- the old "a master-key create must name
        # an owner" refusal existed only because that credential had nobody to
        # assign, and there is no such credential any more.
        ...
        # An ordinary caller may only create a project owned by itself (SPEC
        # §16.2), or by a group its identity provider asserts: it already owns
        # that group's existing projects through `authorize`, so refusing
        # creation would let it reach a group project by transfer but never
        # make one. Naming somebody ELSE is the authority part.
        raise Forbidden("this caller may only create a project it owns")
