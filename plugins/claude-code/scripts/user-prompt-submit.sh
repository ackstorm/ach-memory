#!/usr/bin/env bash
# ach-memory - UserPromptSubmit hook.
#
# UserPromptSubmit takes its context as plain stdout, so this is a plain cat: no
...
# message.
set -u
cat "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/prompt-hint.txt" 2>/dev/null || true
exit 0
